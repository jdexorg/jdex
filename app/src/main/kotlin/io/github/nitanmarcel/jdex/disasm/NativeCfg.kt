...
object NativeCfg {
    ...
    private class NIns(val addr: Int, val mnem: String, val ops: String, val target: Int?, val text: String, val switchTargets: List<Int>, val noReturn: Boolean)
    ...
    fun fromListing(shortId: String, lines: List<String>): MethodCfg? {
        ...
        for (raw in lines) {
            ...
            val noReturn = Mnemonics.isCall(mnem) && NORETURN.containsMatchIn(operand)
            insns.add(NIns(addr, mnem.lowercase(), ops, target, if (ops.isEmpty()) mnem else "$mnem $ops", switchTargets, noReturn))
        }
        ...
        for (i in insns.indices) {
            val ins = insns[i]
            val ends = ins.target != null || Mnemonics.isReturn(ins.mnem, ins.ops) || ins.mnem in UNCOND || ins.mnem in INDIRECT || ins.switchTargets.isNotEmpty() || ins.noReturn
        ...
        for (idx in leaderList.indices) {
            ...
            when {
                ...
                last.noReturn -> {}
                Mnemonics.isReturn(last.mnem, last.ops) || last.mnem in INDIRECT -> {}
